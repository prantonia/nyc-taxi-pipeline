...
class MetadataManager:
    ...
    def get_last_attempted_month(
        self, pipeline_name: str = "incremental"
    ) -> Optional[int]:
        """
    ...
    def get_last_completed_month(
        ...
        """
        Get the last completed month (SUCCESS or SKIPPED) for incremental loads.
        This treats SKIPPED months as completed, allowing pipeline to progress.

        Args:
            pipeline_name: Name of the pipeline (default: incremental)

        Returns:
            Month number (1-12) or None if no completed runs
        """
        try:
            query = f"""
                SELECT month_loaded
            ...
            logger.error(f"Error getting last completed month: {e}")
            return None
    ...
    def get_loaded_months(self) -> List[int]:
        ...
        try:
            ...
            months = [
                month_map[row.month_loaded]
                for row in results
                if row.month_loaded in month_map
            ]
            return months
