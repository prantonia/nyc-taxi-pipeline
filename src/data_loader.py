...
class DataLoader:
    ...
    def check_if_data_exists_in_staging(self, df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            logger.debug(f"Checking for min row: pickup={min_row['tpep_pickup_datetime']}, dropoff={min_row['tpep_dropoff_datetime']}")
            logger.debug(f"Checking for max row: pickup={max_row['tpep_pickup_datetime']}, dropoff={max_row['tpep_dropoff_datetime']}")
            ...
            if exists:
                logger.info(f"Both boundary rows found in staging - data already exists")
            else:
                logger.info(f"Boundary rows check: min_exists={min_exists}, max_exists={max_exists} - data needs upload")
    ...
    def _check_row_exists_in_staging(self, row: pd.Series) -> bool:
        ...
        try:
            ...
            pickup_dt = row['tpep_pickup_datetime'].strftime('%Y-%m-%d %H:%M:%S') if hasattr(row['tpep_pickup_datetime'], 'strftime') else str(row['tpep_pickup_datetime'])
            dropoff_dt = row['tpep_dropoff_datetime'].strftime('%Y-%m-%d %H:%M:%S') if hasattr(row['tpep_dropoff_datetime'], 'strftime') else str(row['tpep_dropoff_datetime'])
    ...
    def download_all_months(self) -> int:
        ...
        try:
            ...
            for month in range(1, 13):
                ...
                logger.info(f"Downloaded data range: {incoming_min} to {incoming_max} ({incoming_rows:,} rows)")
                
                # Check if this exact data already exists using row-based checking
                data_exists = self.check_if_data_exists_in_staging(df)
                
                if data_exists:
                    logger.info(f"{month_name} data already exists in staging (verified by row matching), skipping upload")
                    del df
                    continue
                ...
                logger.info(f"{month_name} uploaded. Total uploaded so far: {total_rows_uploaded:,} rows")
            
            logger.info(f"All months processed. Total uploaded: {total_rows_uploaded:,} rows")
    ...
    def get_staging_row_count_2024(self) -> int:
        ...
        try:
            ...
            logger.info(f"Staging table 2024 row count (date range filter): {count_2024:,}")
    ...
    def should_load_to_raw(self) -> bool:
        ...
        try:
            ...
            if staging_2024_count == raw_count:
                logger.info(f"Raw table in sync ({raw_count:,} rows), skipping recreation")
                return False
            
            logger.info(f"Raw needs update: staging 2024 has {staging_2024_count:,} rows, raw has {raw_count:,} rows")
            return True
