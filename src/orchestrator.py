...
class PipelineOrchestrator:
    ...
    def run_full_refresh(self):
        ...
        try:
            ...
            if raw_rows == 0:
                ...
                logger.info("\n" + "=" * 80)
                logger.info(
                    "FULL REFRESH PIPELINE COMPLETED - SKIPPED (data already loaded)"
                )
    ...
    def run_incremental(self, target_month: Optional[int] = None):
        ...
        try:
            ...
            if month_to_load is None:
                ...
                logger.info("\n" + "=" * 50)
                logger.info(
                    "INCREMENTAL PIPELINE COMPLETED - SKIPPED (all months loaded)"
                )
                logger.info(f"Runtime: {runtime:.2f} seconds")
                logger.info("=" * 50)
            ...
            self.metadata.record_run(
                ...
                rows_loaded=0,
                month=month_to_load
                if target_month or "month_to_load" in locals()
                else None,
                runtime=runtime,
                error_message=error_msg,
            )
    ...
    def _load_staging_to_raw_incremental(self, month: int) -> int:
        ...
        logger.info(
            f"Checking if raw table needs recreation after loading {month_name}..."
        )
