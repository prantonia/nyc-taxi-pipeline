...
Now create a `.env` file to store your configuration using the templates in `.env.example` file. This file will contain sensitive information, which is why we added it to `.gitignore`.
...
The test_config.py file runs automatically before any tests. It sets up the test environment with appropriate flags and dummy values, ensuring tests can import and use the source code without requiring real GCP credentials. The fixtures provide reusable test data and mocked dependencies.
...
Create `tests/test_bigquery_client.py`:
