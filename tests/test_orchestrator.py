...
class TestPipelineOrchestrator:
    ...
    def mock_retry_handler(self):
        ...
        mock = MagicMock()
        mock.retry_operation = MagicMock(
            side_effect=lambda func, name, *args: func(*args) if args else func()
        )
        return mock

    @pytest.fixture
    def orchestrator(
        self, mock_bq_client, mock_data_loader, mock_metadata, mock_retry_handler
    ):
        """Create orchestrator with all mocked dependencies"""
        with patch(
            "src.orchestrator.BigQueryClient", return_value=mock_bq_client
        ), patch("src.orchestrator.DataLoader", return_value=mock_data_loader), patch(
            "src.orchestrator.MetadataManager", return_value=mock_metadata
        ), patch(
            "src.orchestrator.RetryHandler", return_value=mock_retry_handler
        ):
    ...
    def test_full_refresh_staging_already_loaded(
    ...
    def test_full_refresh_skipped_when_raw_in_sync(
        self, orchestrator, mock_data_loader, mock_metadata
    ):
    ...
    def test_full_refresh_failure(self, orchestrator, mock_data_loader, mock_metadata):
        ...
        mock_data_loader.load_full_refresh_to_staging.side_effect = Exception(
            "Download failed"
        )
    ...
    def test_full_refresh_no_staging_data(
        self, orchestrator, mock_data_loader, mock_metadata
    ):
    ...
    def test_incremental_success_first_month(
        ...
        assert result is True
        mock_data_loader.load_incremental_to_staging.assert_called_once_with(
            1
        )  # January
        call_args = mock_metadata.record_run.call_args[1]
        assert call_args["status"] == STATUS_SUCCESS
    ...
    def test_incremental_success_specific_month(
        self, orchestrator, mock_data_loader, mock_metadata
    ):
    ...
    def test_incremental_sequential_months(
        self, orchestrator, mock_data_loader, mock_metadata
    ):
    ...
    def test_incremental_skipped_month_already_loaded(
        self, orchestrator, mock_data_loader, mock_metadata
    ):
    ...
    def test_incremental_failure(self, orchestrator, mock_data_loader, mock_metadata):
        ...
        mock_data_loader.load_incremental_to_staging.side_effect = Exception(
            "Network error"
        )
    ...
    def test_load_staging_to_raw_full_needs_recreation(
        self, orchestrator, mock_data_loader, mock_bq_client
    ):
    ...
    def test_load_staging_to_raw_incremental_calculates_delta(
        self, orchestrator, mock_data_loader, mock_bq_client
    ):
        """Test incremental load calculates rows added correctly"""
        mock_data_loader.should_load_to_raw.return_value = True
        mock_data_loader.get_raw_row_count.side_effect = [
            500000,
            600000,
        ]  # Before/after
    ...
    def test_transform_to_silver(self, orchestrator, mock_bq_client):
        ...
        mock_bq_client.execute_sql_file.assert_called_once()
        assert "create_silver_table.sql" in str(
            mock_bq_client.execute_sql_file.call_args
        )
...
class TestPipelineIntegration:
    ...
    def orchestrator_integration(self):
        ...
        with patch("src.orchestrator.BigQueryClient") as mock_bq, patch(
            "src.orchestrator.DataLoader"
        ) as mock_loader, patch("src.orchestrator.MetadataManager") as mock_meta, patch(
            "src.orchestrator.RetryHandler"
        ) as mock_retry:
            # Setup basic returns
            ...
            mock_retry_instance.retry_operation.side_effect = (
                lambda func, name, *args: func(*args) if args else func()
            )
